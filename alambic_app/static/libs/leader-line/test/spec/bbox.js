            ...
            it(title, function (done) {
                ...
                loadPage('spec/bbox/coordinates.html', function (window, document, body) {
                    ...
                    ['static', 'absolute'].forEach(function (div, i) {
                        var len = condition.expected[div].reduce(function (sum, prop) {
                            return (sum += DOC_LEN[prop]);
                        }, 0);
                            ...
                            function expandProps(frames, div) {
                                ...
                                function addAbsolute(win) {
                                    if (condition.indexOf(win + ':body-relative') > -1) {
                                        props = props.concat(REL_BODY.map(function (prop) {
                                            return win + ':' + prop;
                                        }));
                                    } else if (condition.indexOf(win + ':html-relative') > -1) {
                                        props = props.concat(REL_HTML.map(function (prop) {
                                            return win + ':' + prop;
                                        }));
                                    }
                                }
                                ...
                                frames.forEach(function (frame, i) {
                                    ...
                                    if (i < frames.length - 1) { // iframe coordinates
                                        ...
                                        if (div === 'static') {
                                            props = props.concat(
                                                REL_BODY.concat(['body-padding']).map(function (prop) {
                                                    return frame + ':' + prop;
                                                }));
                                        } else {
                                            addAbsolute(frame);
                                    ...
                                    expandProps(item[key + 'WinPath'], item[key + 'Div'])
                                        .reduce(function (bBox, prop) {
                                                ...
                                                if (condition.indexOf(prop) > -1 ||
                                                    ...
                                                    if (!leftTop || leftTop === 'left') {
                                                        bBox.left += DOC_LEN[targetProp];
                                                    }
                                                    if (!leftTop || leftTop === 'top') {
                                                        bBox.top += DOC_LEN[targetProp];
                                                    }
                                                }
                                                return bBox;
                                            },
                ...
                loadPage('spec/bbox/scroll.html', function (window, document) {
                    ...
                    if (scroll) {
                        ...
                        ['overflow1', 'overflow1_overflow2', 'iframe1_overflow1', 'iframe1_overflow1_overflow2']
                            .forEach(function (overflow) {
                                elms[overflow].scrollLeft = elms[overflow].scrollTop = scroll;
                            });
                    }
